...
from apps.event_processor.rule_engine import (
    AlertRule,
    CooldownSuppressor,
    RuleEngine,
...
from shared.contracts.alert_event import AlertSeverity
...
# Registry mapping rule "type" strings to builder functions.
# Each builder takes a rule config dict and returns an AlertRule instance.
# To add a new rule type, implement the AlertRule subclass in rule_engine.py,
# then register a builder here.
_RULE_BUILDERS: dict[str, type] = {
    "threshold": ThresholdAlertRule,
    "sustained": SustainedDetectionRule,
}


def _build_rule(rule_config: dict) -> AlertRule | None:
    """Build a single AlertRule from a config dict.

    Args:
        rule_config: Dict with "type" key and rule-specific parameters.

    Returns:
        An AlertRule instance, or None if the type is unknown.
    """
    rule_type = rule_config.get("type")
    if rule_type not in _RULE_BUILDERS:
...
def build_rule_engine(config: dict) -> RuleEngine:
    """Create a RuleEngine from a rule config dictionary.

    Args:
        config: Dict with "rules" list and optional "cooldown_sec".
    ...
    suppressor = CooldownSuppressor(cooldown_sec=cooldown_sec)
...
def load_rules(
    ...
    logger.info(
        ...
        rule_count=len(default_config.get("rules", [])),
        cooldown_sec=default_config.get("cooldown_sec", 30.0),
    )

    # Load camera-specific configs
    per_camera: dict[str, RuleEngine] = {}
    for config_file in sorted(rules_dir.glob("*.json")):
        if config_file.name == "default.json":
            continue

        camera_id = config_file.stem  # e.g., "cam_01.json" → "cam_01"
        try:
